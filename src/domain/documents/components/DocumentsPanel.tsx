  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2">
        {/* Kosongkan kiri agar tidak ada duplikasi dari parent */}
        <div className="ml-auto flex items-center gap-2">
          <SearchDocument />
          <AddDocumentButton archives={uiArchives} onSaved={onAddSaved} />
          <ViewToggle initialMode="list" onChange={setMode} />
        </div>
      </div>
        ...
        <div className="p-3">
          ...
          <div className="mt-3">
            {mode === "list" ? (
              <DocumentTable
                items={items}
                meta={meta ?? ({ page: 1, totalPages: 1 } as ListMeta)}
              />
            ) : (
              <DocumentCardGrid items={items as unknown as GridItems} />
            )}
          </div>
