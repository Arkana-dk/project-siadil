...
import type { DashboardData, ReminderItem, ReminderSeverity } from "@/types/dashboard";
...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  // 1 kartu archive paling baru (biar sama seperti desain)
  ...
  // reminders (urut: danger dulu, lalu warn, lalu due_date asc)
  const remindersRows = await query<{
    id: string;
    severity: string; // bisa "danger"/"warn" atau tipe lain di DB
    title: string;
  ...
  const reminders: ReminderItem[] = remindersRows.map((r): ReminderItem => {
    const sev: ReminderSeverity = r.severity === "danger" ? "danger" : "warn";
    return { id: r.id, severity: sev, title: r.title, subtitle: r.subtitle, desc: r.desc };
  });
